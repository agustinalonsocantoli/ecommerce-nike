export const Feature = ({ ifExists ,endpoint }) => {
    return(
        <div className={`flex items-center justify-between lg:flex-col lg:justify-center nike-container ${ifExists ? 'flex-row-reverse' : 'flex-row'}`}>
            <div className="max-w-lg lg:max-w-none w-full md:text-center grid items-center md:justify-items-center">
                ...
                <h1 className="text-5xl lg:text-4xl md:text-3xl sm:text-2xl font-bold text-slate-900 filter drop-shadow-lg">{endpoint.title}</h1>
                <p className="xl:text-sm my-4 text-slate-900">{endpoint.text}</p>
                <a href={endpoint.url} className="flex items-center" target={'_blank'} role='button' rel='noopener noreferrer'>
                    <button type="button" className="button-theme bg-slate-900 shadow-slate-900 text-slate-100 py-1.5">{endpoint.btn}</button>
                </a>
            </div>
